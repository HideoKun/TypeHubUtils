...
it("should work for args", () => {
  expectTypeOf<TestStr>().toEqualTypeOf<TrimStart<`${WHITE_SPACE}${TestStr}`>>();
  expectTypeOf<TestStr>().toEqualTypeOf<TrimStart<`${WHITE_SPACE}${WHITE_SPACE}${TestStr}`>>();
});
...
it("should return never", () => {
  expectTypeOf<TrimStart<string>>().toBeNever();
});
